...
client.on('error', (err) => {
  ...
  console.log('Revisa la configuració de connexió al servidor MQTT (host, port, usuari i contrasenya).');
});
...
async function initVectorLlicencia(Llicencia, Empresa, dataInici = null) {
  ...
  try {
    if (dataInici == null && estocPerLlicencia[Llicencia] && estocPerLlicencia[Llicencia]['LastUpdate'] && new Date(estocPerLlicencia[Llicencia]['LastUpdate']).toDateString() === avui.toDateString()) return;
    console.log('🔄');
    ...
    if (Empresa.toLowerCase() === 'fac_demo' || Empresa.toLowerCase() === 'fac_camps') {
      // Definim el bucle des d'ahir fins a 31 dies després d'avui
    ...
    sqlSt = `use ${Empresa} 
        IF EXISTS (SELECT * FROM sys.tables WHERE name = '${nomTaulaCompromiso(avui)}') And EXISTS (SELECT * FROM sys.tables WHERE name = '${nomTaulaVenut(avui)}') And EXISTS (SELECT * FROM sys.tables WHERE name = '${nomTaulaVenut(
      new Date(lastWeekSameDay),
        ...
        BEGIN   
          ...
          FROM 
              ...
              WHERE dia = '${moment(avui).format('YYYY-MM-DD')}' AND botiga =  ${Llicencia}) o
          JOIN 
          [${nomTaulaVenut(avui)}] v ON v.plu = o.plu AND v.Botiga =  ${LlicenciaA} AND DAY(v.data) = ${moment().date()}
          GROUP BY 
          ...
          FROM 
              ...
              WHERE dia = '${moment(avui).format('YYYY-MM-DD')}' AND botiga =  ${Llicencia}) o
          JOIN 
          [${nomTaulaVenut(new Date(lastWeekSameDay))}] v ON v.plu = o.plu AND v.Botiga =  ${LlicenciaA}  AND DAY(v.data) = ${lastWeekSameDayDia} 
          GROUP BY 
              (DATEDIFF(MINUTE, CAST(v.data AS DATE), v.data) / 30),
              objectiu,
              v.plu
    ...
    sqlSt = `use ${Empresa} 
          ...
          select 
                ...
                FROM 
                [${nomTaulaVenut(avui)}] v where v.Botiga =  ${LlicenciaA} AND DAY(v.data) = ${moment().date()}
                GROUP BY 
                    (DATEDIFF(MINUTE, CAST(v.data AS DATE), v.data) / 30)
                  UNION ALL
                  -- Subconsulta per les dades "Passat"
                SELECT 
                    ...
                    SUM(v.import) AS import
                FROM 
                [${nomTaulaVenut(new Date(lastWeekSameDay))}] v where v.Botiga =  ${LlicenciaA}  AND DAY(v.data) = ${lastWeekSameDayDia}
                GROUP BY 
                    (DATEDIFF(MINUTE, CAST(v.data AS DATE), v.data) / 30)
                ) a 
                GROUP BY 
  ...
  if (data && data.Articles && Array.isArray(data.Articles)) {
    try {
      ...
      data.Articles.forEach((article) => {
        // actualitzem les dades a l estructura d articles controlats
        let articleCodi = 0;
        ...
        switch (tipus) {
          ...
          case 'Encarreg':
            if (estocPerLlicencia[data.Llicencia][articleCodi])
              estocPerLlicencia[data.Llicencia][articleCodi].unitatsEncarregades = parseFloat((parseFloat(article.Quantitat) + parseFloat(estocPerLlicencia[data.Llicencia][articleCodi].unitatsEncarregades)).toFixed(3));
            break;
        }
      });

      Object.values(estocPerLlicencia[data.Llicencia]).forEach((controlat) => {
        // Revisem els indicadors
        missatge = controlat.ultimMissatge; // Creem el missatge
        if (controlat.tipus === 'Encarrecs') {
          // Si hi ha encarregs
          controlat.estoc = parseFloat(controlat.unitatsServides) - parseFloat(controlat.unitatsVenudes) - parseFloat(controlat.unitatsEncarregades);
